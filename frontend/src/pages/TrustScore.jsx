...
function TrustScore() {
  ...
  const downloadReport = () => {
    ...
    a.href = url;
    a.download = 'squadmind-trustscore-report.csv';
    a.click();
  ...
  const componentDetails = {
    ...
    'REPEAT CUSTOMERS': { desc: 'Tracks the percentage of customers who return to buy again. Higher repeat rate = stronger loyalty = lower credit risk.', tip: 'Consider a loyalty program to push this score from 18 to 20.' },
    ...
    'CASHFLOW STABILITY': { desc: 'Analyzes how consistently your cash flow moves. Erratic cash flow raises lender risk perception.', tip: 'Resolve the Nov 1-7 dip risk to improve this component.' },
    'GROWTH TREND': { desc: 'Measures revenue trajectory over the last 90 days. Currently low due to seasonal Q4 adjustments.', tip: 'A single strong month can bring this from 0 to 10+. Focus on Nov sales push.' },
  };
        ...
        <div className="space-y-5">
          <div className="p-6 bg-gradient-to-br from-[#001f3f] to-[#0ea5e9] rounded-2xl text-white text-center">
            ...
            <div className="grid grid-cols-3 gap-3 mt-6">
              <div>
                <p className="text-xl font-black">0.24%</p>
                <p className="text-[10px] text-white/60">Fraud Rate</p>
              </div>
              <div>
                <p className="text-xl font-black">+{revenueChange}%</p>
                <p className="text-[10px] text-white/60">Revenue Growth</p>
              </div>
              <div>
                <p className="text-xl font-black">90</p>
                <p className="text-[10px] text-white/60">Days Analyzed</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            {[
              { platform: 'Copy Report Link', icon: '🔗', action: () => { navigator.clipboard?.writeText('https://squadmind-khaki.vercel.app/trustscore'); setActiveModal(null); showSuccess('Report link copied to clipboard!'); } },
              { platform: 'Email to Lender', icon: '📧', action: () => { setActiveModal(null); showSuccess('Email draft opened. Attach your report PDF.'); } },
              { platform: 'Download PDF Report', icon: '📥', action: () => { downloadReport(); setActiveModal(null); } },
              { platform: 'WhatsApp', icon: '💬', action: () => { setActiveModal(null); showSuccess('Opening WhatsApp with your TrustScore card...'); } },
            ].map((item, i) => (
              <button
                key={i}
                onClick={item.action}
                className="w-full flex items-center gap-3 p-4 bg-slate-50 hover:bg-cyan-50 border border-transparent hover:border-cyan-100 rounded-2xl transition-all cursor-pointer text-left"
              >
        ...
        <div className="space-y-5">
          ...
          <div className="relative h-32 flex items-end gap-2">
            {[
              ...
              { month: 'Mar', score: 70 },
              { month: 'Apr', score: 72 },
              { month: 'May', score: score, current: true },
            ].map((item, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[10px] font-black text-slate-900">{item.score}</span>
                <div
                  className={`w-full rounded-t-lg ${item.current ? 'bg-[#001f3f]' : 'bg-slate-200'}`}
                  style={{ height: `${(item.score / 100) * 100}%` }}
                ></div>
                <span className={`text-[10px] font-bold ${item.current ? 'text-[#001f3f]' : 'text-slate-400'}`}>{item.month}</span>
              </div>
            ))}
          </div>
          <div className="space-y-3">
            ...
            {[
              { month: 'May 2026', change: '+2', reason: 'Improved fraud detection rate', color: 'text-emerald-600' },
              { month: 'Apr 2026', change: '+2', reason: 'Revenue consistency improved', color: 'text-emerald-600' },
              { month: 'Mar 2026', change: '+2', reason: 'New repeat customers added', color: 'text-emerald-600' },
              { month: 'Feb 2026', change: '+3', reason: 'Cash flow stabilized after Q1', color: 'text-emerald-600' },
              { month: 'Jan 2026', change: '+4', reason: 'First full month of Squad data', color: 'text-emerald-600' },
            ].map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                <div>
                  <p className="text-xs font-bold text-slate-900">{item.month}</p>
                  <p className="text-[10px] text-slate-400">{item.reason}</p>
                </div>
                <span className={`text-sm font-black ${item.color}`}>{item.change}</span>
              </div>
            ))}
          </div>
          <div className="p-4 bg-cyan-50 rounded-2xl border border-cyan-100">
            <p className="text-xs text-slate-600 leading-relaxed">
              <span className="font-bold text-[#00d2ff]">Projection:</span> At your current growth rate, you could reach 80/100 by August 2026, unlocking premium lender rates.
            </p>
          </div>
        </div>
      </Modal>
          ...
          <div className="space-y-5">
              ...
              <div className="w-full h-3 bg-slate-200 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${
                    (selectedComponent.color || getComponentColor(selectedComponent)) === 'red' ? 'bg-red-500' :
                    (selectedComponent.color || getComponentColor(selectedComponent)) === 'emerald' ? 'bg-emerald-500' : 'bg-[#00d2ff]'
                  }`}
                  style={{ width: getProgress(selectedComponent) }}
                ></div>
              </div>
            </div>
            <div className="p-4 bg-cyan-50 rounded-2xl border border-cyan-100">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] font-bold text-[#00d2ff] uppercase tracking-wider mb-2">What This Measures</p>
                  <p className="text-sm text-slate-600 leading-relaxed">
                    {componentDetails[selectedComponent.label || selectedComponent.name]?.desc || 'This component measures a key financial health indicator used in your overall TrustScore calculation.'}
                  </p>
                </div>
              </div>
            </div>
            <div className="p-4 bg-[#001f3f] rounded-2xl text-white">
              ...
              <p className="text-sm text-slate-300 leading-relaxed">
                {componentDetails[selectedComponent.label || selectedComponent.name]?.tip || 'Focus on consistent transaction patterns and maintaining low fraud rates to improve this component.'}
              </p>
            </div>
              ...
              <div className="p-4 bg-red-50 rounded-2xl border border-red-100">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-xs text-red-700 leading-relaxed">
                    <span className="font-bold">Action Required:</span> This component is currently at 0. Improving it could add up to {selectedComponent.max ?? 15} points to your TrustScore.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="p-4 md:p-8 max-w-[1400px] w-full mx-auto">
          ...
          <div className="mb-12">
            <div className="flex justify-between items-end mb-6">
              ...
              <button
                onClick={() => setActiveModal('history')}
                className="text-xs font-bold text-[#001f3f] hover:underline transition-all cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              {scoreComponents.map((metric, i) => {
                ...
                return (
                  <div
                    key={i}
                    onClick={() => { setSelectedComponent(metric); setActiveModal('component'); }}
                    className={`bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col cursor-pointer hover:shadow-md hover:border-[#00d2ff]/30 transition-all group ${color === 'red' ? 'border-l-4 border-l-red-500' : ''}`}
                  >
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-4 leading-tight group-hover:text-[#00d2ff] transition-colors">{metric.label || metric.name}</p>
                    ...
                    <p className="text-[9px] text-[#00d2ff] font-bold mt-3 opacity-0 group-hover:opacity-100 transition-opacity">Click for details →</p>
                  </div>
                );
              })}
            </div>
