...
import {
  Sparkles, Shield, TrendingUp, Zap, ArrowRight,
...
export default function Landing() {
  ...
  const features = [
    {
      ...
      tag: 'PREDICTIVE',
      iconBg: 'bg-orange-50 text-[#E8762E]',
    },
    {
  ...
  return (
    <div className="bg-[#f8fafc] text-[#001f3f] min-h-screen font-outfit overflow-x-hidden">

      {/* ── NAV ── */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-xl shadow-sm py-3' : 'bg-white py-5'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-lg font-black text-[#001f3f] tracking-tight">SquadMind</span>
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Features', 'Benefits', 'TrustScore', 'Pricing'].map(item => (
              <a key={item} href="#" className="text-sm font-medium text-slate-500 hover:text-[#001f3f] transition-colors">{item}</a>
            ))}
          </div>

          <div className="hidden md:flex items-center gap-4">
            <button onClick={() => navigate('/login')} className="text-sm font-bold text-slate-500 hover:text-[#001f3f] transition-colors cursor-pointer">
              Sign In
            </button>
            <button
              onClick={() => navigate('/register')}
              className="px-5 py-2.5 bg-[#E8762E] text-white font-black text-sm rounded-xl hover:bg-[#E8762E]/90 transition-all shadow-md shadow-[#E8762E]/20 cursor-pointer"
            >
              Get Started Free
            </button>
          </div>

          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden text-slate-500 cursor-pointer">
        ...
        {menuOpen && (
          <div className="md:hidden bg-white border-t border-slate-100 px-6 py-6 space-y-4 shadow-lg">
            {['Features', 'Benefits', 'TrustScore', 'Pricing'].map(item => (
              <a key={item} href="#" className="block text-sm font-medium text-slate-600">{item}</a>
            ))}
      ...
      {/* ── HERO ── */}
      <section className="relative min-h-screen flex items-center pt-24 pb-20 px-6 overflow-hidden bg-white">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_20%_50%,rgba(232,118,46,0.05)_0%,transparent_60%)]"></div>
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_80%_20%,rgba(0,31,63,0.03)_0%,transparent_60%)]"></div>
        <div className="absolute inset-0 opacity-[0.015]" style={{backgroundImage: 'linear-gradient(rgba(232,118,46,1) 1px, transparent 1px), linear-gradient(90deg, rgba(232,118,46,1) 1px, transparent 1px)', backgroundSize: '60px 60px'}}></div>
        ...
        <div className="max-w-7xl mx-auto w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black leading-[1.0] tracking-tight mb-8 text-[#001f3f]">
                The AI CFO
                <span className="block text-[#E8762E]">Built for Your</span>
                <span className="block text-slate-400">Business.</span>
              </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-12">
                <button
                  onClick={() => navigate('/register')}
                  className="flex items-center justify-center gap-3 px-8 py-4 bg-[#E8762E] text-white font-black text-sm rounded-2xl hover:bg-[#E8762E]/90 transition-all shadow-xl shadow-[#E8762E]/20 hover:scale-105 cursor-pointer uppercase tracking-wider"
                >
                  <Zap className="w-4 h-4" /> Connect My Squad Account
                </button>
                <button
                  onClick={() => navigate('/login')}
                  className="flex items-center justify-center gap-3 px-8 py-4 bg-slate-100 border border-slate-200 text-[#001f3f] font-bold text-sm rounded-2xl hover:bg-slate-200 transition-all cursor-pointer"
                >
              ...
              <div className="flex items-center gap-6 pt-6 border-t border-slate-100">
                <div className="flex -space-x-2">
                  {['AO', 'EN', 'CE', 'BL', 'MO'].map((init, i) => (
                    <div key={i} className="w-8 h-8 rounded-full bg-gradient-to-br from-[#E8762E] to-[#001f3f] border-2 border-white flex items-center justify-center text-[9px] font-black text-white">{init}</div>
                  ))}
                </div>
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">Trusted by <span className="text-[#001f3f] font-bold">4,200+</span> Squad merchants</p>
                </div>
              </div>
            </div>
            ...
            <div className="relative hidden lg:block">
              <div className="relative bg-[#001f3f] rounded-3xl border border-[#001f3f]/10 p-6 shadow-2xl shadow-[#001f3f]/10">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">LIVE DASHBOARD</p>
                ...
                <div className="bg-[#E8762E]/10 border border-[#E8762E]/20 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div className="absolute -bottom-4 -left-4 bg-white border border-slate-200 text-[#001f3f] text-xs font-black px-4 py-2 rounded-xl shadow-lg -rotate-2">
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {[
              ...
              { value: `${counter.revenue}%`, label: 'Average Confidence Score', color: 'text-emerald-600' },
              { value: `₦${counter.threats}k`, label: 'Losses Prevented by AI', color: 'text-purple-600' },
            ].map((stat, i) => (
              <div key={i} className="flex flex-col items-center gap-2">
                <span className={`text-4xl md:text-5xl font-black ${stat.color}`}>{stat.value}</span>
                <span className="text-sm text-slate-500 font-medium uppercase tracking-widest">{stat.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 border border-slate-200 rounded-full text-slate-500 text-xs font-black uppercase tracking-widest mb-6">
              <BarChart3 className="w-3.5 h-3.5" /> Everything You Need
            </div>
            <h2 className="text-4xl md:text-5xl font-black text-[#001f3f] mb-6 leading-tight">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feat, i) => (
              <div
                key={i}
                className="relative bg-[#f8fafc] border border-slate-100 rounded-3xl p-8 group hover:scale-[1.02] hover:shadow-lg hover:border-[#E8762E]/20 transition-all duration-300 cursor-pointer"
              >
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest border border-slate-200 bg-white px-2 py-1 rounded-lg">{feat.tag}</span>
                  </div>
                  <h3 className="text-lg font-black text-[#001f3f] mb-3">{feat.title}</h3>
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black text-[#001f3f] mb-4">From data to decision<br /><span className="text-[#E8762E]">in seconds.</span></h2>
            <p className="text-slate-500 text-lg font-medium">No technical setup. No bank visit. Just your Squad account.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="relative">
                {i < 3 && <div className="hidden md:block absolute top-8 left-full w-full h-px bg-gradient-to-r from-[#E8762E]/30 to-transparent z-0"></div>}
                <div className="relative z-10 bg-white border border-slate-100 rounded-3xl p-8 hover:border-[#E8762E]/30 hover:shadow-md transition-all">
                  ...
                  <h3 className="text-base font-black text-[#001f3f] mb-3">{item.title}</h3>
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="bg-[#001f3f] rounded-3xl p-8 shadow-2xl shadow-[#001f3f]/10">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6">SAMPLE TRUSTSCORE REPORT</p>
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black text-[#001f3f] mb-4">What merchants say</h2>
            <p className="text-slate-500 text-lg font-medium">Real results from real Squad businesses across Nigeria.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div key={i} className="bg-white border border-slate-100 rounded-3xl p-8 hover:border-[#E8762E]/20 hover:shadow-md transition-all">
                <div className="flex items-center gap-1 mb-6">
                  {[...Array(t.stars)].map((_, si) => <Star key={si} className="w-4 h-4 fill-yellow-400 text-yellow-400" />)}
                </div>
                <p className="text-slate-600 text-sm leading-relaxed font-medium mb-8 italic">"{t.quote}"</p>
                <div className="flex items-center gap-3 pt-6 border-t border-slate-100">
                  ...
                  <div>
                    <p className="text-sm font-black text-[#001f3f]">{t.name}</p>
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="bg-[#001f3f] rounded-[48px] p-12 md:p-16 text-center relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_0%,rgba(232,118,46,0.1)_0%,transparent_70%)]"></div>
            <div className="relative z-10">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#E8762E]/10 border border-[#E8762E]/20 rounded-full text-[#E8762E] text-xs font-black uppercase tracking-widest mb-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {[
                  ...
                  <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs font-bold text-slate-300 text-center">{item.label}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-5xl md:text-6xl font-black text-[#001f3f] mb-6 leading-tight">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={() => navigate('/register')}
              className="flex items-center justify-center gap-3 px-10 py-5 bg-[#E8762E] text-white font-black text-base rounded-2xl hover:bg-[#E8762E]/90 transition-all shadow-xl shadow-[#E8762E]/20 hover:scale-105 cursor-pointer uppercase tracking-wider"
            >
              <Zap className="w-5 h-5" /> Launch My AI CFO
            </button>
            <button
              onClick={() => navigate('/login')}
              className="flex items-center justify-center gap-3 px-10 py-5 bg-white border border-slate-200 text-[#001f3f] font-bold text-base rounded-2xl hover:bg-slate-50 transition-all cursor-pointer shadow-sm"
            >
              Already have an account
            </button>
          </div>
          <p className="text-slate-400 text-sm mt-6 font-medium">Free to start · No credit card required · Powered by Squad API</p>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-100 bg-white py-16 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
            <div className="md:col-span-2">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-lg font-black text-[#001f3f]">SquadMind AI</span>
              </div>
            ...
            {[
              ...
              <div key={i}>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">{col.title}</p>
                <ul className="space-y-3">
                  {col.links.map(link => (
                    <li key={link}><a href="#" className="text-sm text-slate-500 hover:text-[#001f3f] transition-colors font-medium">{link}</a></li>
                  ))}
                </ul>
              </div>
            ))}
          ...
          <div className="border-t border-slate-100 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-xs text-slate-400 font-medium">© 2026 SquadMind. Powered by Squad · HabariPay · GTCO Ecosystem</p>
            <p className="text-xs text-slate-400 font-medium uppercase tracking-widest">Squad Hackathon 3.0 — Smart Systems: The Intelligent Economy</p>
          </div>
        </div>
      </footer>
    </div>
