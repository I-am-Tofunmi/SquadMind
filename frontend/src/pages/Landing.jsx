...
function Landing() {
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col font-outfit text-slate-900 overflow-x-hidden">
      ...
      {/* Navbar */}
      <nav className="w-full h-20 bg-[#E8762E] flex items-center justify-between px-6 md:px-20 relative z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="bg-[#001f3f] text-white px-5 md:px-6 py-2.5 rounded-lg text-[13px] font-bold hover:bg-[#001f3f]/90 transition-all cursor-pointer shadow-lg"
          >
            Get Started
          </button>
          <button 
        ...
        {isMenuOpen && (
          <div className="absolute top-20 left-0 w-full bg-[#E8762E] border-t border-white/5 p-6 flex flex-col gap-6 md:hidden shadow-2xl z-50">
      ...
      <section className="pt-16 md:pt-24 pb-20 md:pb-32 px-6 flex flex-col items-center text-center">
        <div className="inline-flex items-center px-4 py-1.5 bg-[#001f3f]/10 rounded-full mb-8">
           <span className="text-[10px] font-extrabold text-[#001f3f] uppercase tracking-wider">AI DRIVEN BUSINESS INTELLIGENCE</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mb-16 md:mb-24 w-full sm:w-auto">
          <button 
            onClick={handleGetStarted}
            className="w-full sm:w-auto bg-[#001f3f] text-white px-8 py-4 rounded-xl font-bold text-sm hover:translate-y-[-2px] hover:bg-[#001f3f]/90 transition-all cursor-pointer shadow-xl shadow-[#001f3f]/20"
          >
            Connect My Squad Account
          </button>
          <button className="w-full sm:w-auto bg-card border border-slate-200 text-[#E8762E] px-10 py-4 rounded-xl font-bold text-sm hover:bg-slate-50 transition-all shadow-sm cursor-pointer">
            View Demo
          </button>
        </div>

        {/* Browser Mockup */}
        <div className="w-full max-w-5xl mx-auto bg-card rounded-[24px] md:rounded-[32px] shadow-[0_32px_64px_-16px_rgba(232,118,46,0.1)] border border-slate-100 p-2 md:p-3 relative">
                 ...
                 <div className="w-16 md:w-56 bg-[#E8762E] h-full p-4 md:p-8 space-y-8 shrink-0 flex flex-col">
                    ...
                    <div className="space-y-4">
                       {[
                         ...
                         <div key={i} className="flex items-center gap-3">
                            <div className={`w-4 h-4 rounded-md ${item.active ? 'bg-[#001f3f]' : 'bg-white/10'}`}></div>
                    ...
                    <div className="mt-auto space-y-4">
                       ...
                       <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-[#001f3f]"></div>
                 ...
                 <div className="flex-1 bg-white p-6 md:p-10 flex flex-col gap-8">
                    <div className="flex justify-between items-center pb-6 border-b border-slate-50">
                       ...
                       <div className="flex gap-4">
                          <div className="w-24 h-6 bg-[#001f3f]/5 rounded-full hidden sm:block"></div>
                          <div className="w-8 h-8 bg-slate-50 rounded-full"></div>
                       </div>
                    </div>

                    <div className="space-y-2">
                       <div className="w-48 h-6 bg-[#E8762E]/5 rounded-md"></div>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                       {[
                         ...
                         <div key={i} className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm space-y-4">
                            ...
                            {card.bar && <div className="w-full h-1.5 bg-slate-100 rounded-full"><div className="w-[72%] h-full bg-[#001f3f] rounded-full"></div></div>}
                         </div>
                       ))}
                    </div>
                       ...
                       <div className="md:col-span-2 bg-[#f8fafc] rounded-2xl border border-slate-100 p-6 flex flex-col justify-end gap-6 overflow-hidden">
                          <div className="flex items-end gap-3 h-24">
                             {[40, 60, 45, 80, 55, 90, 70].map((h, i) => (
                               <div key={i} className="flex-1 bg-[#001f3f]/20 rounded-t-lg transition-all hover:bg-[#001f3f]/40" style={{ height: `${h}%` }}></div>
                             ))}
                          </div>
                       </div>
      ...
      <section id="features" className="py-16 md:py-20 px-6 max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { icon: <BarChart className="w-5 h-5 text-[#001f3f]" />, title: "Revenue Intelligence", text: "Identify growth patterns and revenue leakage automatically. SquadMind analyzes every transaction to find hidden opportunities for sustainable scale." },
          { icon: <Shield className="w-5 h-5 text-[#001f3f]" />, title: "Fraud Detection", text: "Stop suspicious activities before they impact your bottom line. Our AI monitors patterns in real-time to safeguard your business revenue." },
          { icon: <Activity className="w-5 h-5 text-[#001f3f]" />, title: "Cash Flow Forecast", text: "Predict your financial health 90 days out. Smart forecasting helps you make confident decisions about hiring, inventory, and scaling." },
          { icon: <ShieldCheck className="w-5 h-5 text-[#001f3f]" />, title: "TrustScore", text: "Turn your Squad transaction history into a verified credit score. Access loans and financial services for the first time." }
        ].map((f, i) => (
          <div key={i} className="bg-card p-6 md:p-8 rounded-[24px] md:rounded-[32px] shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-500 hover:translate-y-[-4px]">
            <div className="w-10 h-10 md:w-12 md:h-12 bg-[#001f3f]/5 rounded-2xl flex items-center justify-center mb-6 md:mb-8">
              {f.icon}
            </div>
      ...
      <section id="intelligence" className="py-20 md:py-32 px-6 max-w-7xl mx-auto">
        <div className="bg-white rounded-[32px] md:rounded-[40px] p-8 md:p-20 shadow-sm border border-slate-100 flex flex-col lg:flex-row items-center gap-12 md:gap-20">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center gap-4">
                  <div className="w-5 h-5 rounded-full bg-[#001f3f]/5 flex items-center justify-center">
                    <CheckCircle2 className="w-3 h-3 text-[#001f3f]" />
                  </div>
          ...
          <div className="flex-1 w-full max-w-md bg-white rounded-[24px] md:rounded-[32px] shadow-2xl shadow-[#E8762E]/10 border border-slate-100 p-6 md:p-8 relative overflow-hidden">
             <div className="flex justify-between items-start mb-6 md:mb-8 relative z-10">
                ...
                <div className="w-8 h-8 md:w-10 md:h-10 bg-[#001f3f]/5 rounded-2xl flex items-center justify-center">
                   <Activity className="w-4 h-4 md:w-5 md:h-5 text-[#001f3f]" />
                </div>
             </div>
             <div className="h-32 md:h-40 w-full relative z-10">
                <svg className="w-full h-full" viewBox="0 0 400 150">
                  ...
                  <path d="M0 120 Q 50 130 100 100 T 200 110 T 300 80 T 400 100" fill="none" stroke="#001f3f" strokeWidth="4" strokeLinecap="round" />
                </svg>
             </div>
          </div>
        </div>
      ...
      <footer className="mt-auto py-12 px-6 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left">
        ...
        <button 
           onClick={handleGetStarted}
           className="px-8 py-3 border border-[#E8762E] rounded-full text-[10px] font-black text-[#E8762E] uppercase tracking-widest hover:bg-[#E8762E] hover:text-white transition-all cursor-pointer"
        >
           Connect Now
        </button>
      </footer>
