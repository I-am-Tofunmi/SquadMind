...
function Alerts() {
  ...
  const alerts = [
    ...
    { id: 2, type: 'PERFORMANCE PEAK', color: '#0ea5e9', title: 'Your Best Month Yet 🥳', text: 'You are up 23% from last month. Your Friday flash promos are working. Well done o!', time: '1d ago', buttons: ['Share Stats'], modal: 'share' },
  ...
  return (
    <div className="flex h-screen w-full bg-[#f8fafc] font-outfit text-slate-900 overflow-hidden relative">
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            {[
              ...
              <div key={i} className="flex items-start gap-2 p-3 bg-cyan-50 rounded-xl">
                <CheckCircle2 className="w-4 h-4 text-[#00d2ff] mt-0.5 shrink-0" />
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-2xl p-4">
              ...
              <p className="text-xl font-bold text-red-600">₦12,500</p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-4">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Time</p>
              <p className="text-xl font-bold text-slate-900">2:14 AM</p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-4">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Terminal</p>
              <p className="text-sm font-bold text-slate-900">POS-221</p>
            </div>
            <div className="bg-red-50 rounded-2xl p-4">
              <p className="text-[10px] font-bold text-red-400 uppercase tracking-wider mb-1">Risk Level</p>
          ...
          <div className="p-4 bg-cyan-50 rounded-2xl border border-cyan-100">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 leading-relaxed">This reversal occurred at an unusual time (2:14 AM) from a new device location not previously associated with POS-221. Pattern matches known fraud signatures in our database.</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => { setActiveModal(null); showSuccess('Reversal approved as legitimate.'); }}
      ...
      <Modal isOpen={activeModal === 'share'} onClose={() => setActiveModal(null)} title="Share Your Stats">
        <div className="space-y-5">
          <div className="p-6 bg-gradient-to-br from-[#001f3f] to-[#0ea5e9] rounded-2xl text-white text-center">
            ...
            <div className="grid grid-cols-3 gap-3 mt-6">
              <div>
                <p className="text-xl font-black">1,247</p>
              ...
              <div>
                ...
                <p className="text-[10px] text-white/60">Trust Score</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            {[
              { platform: 'Copy Link', icon: '🔗', action: () => { navigator.clipboard?.writeText('https://squadmind-khaki.vercel.app'); showSuccess('Link copied to clipboard!'); setActiveModal(null); } },
              ...
              <button
                key={i}
                onClick={item.action}
                className="w-full flex items-center gap-3 p-4 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-colors cursor-pointer"
              >
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            {[
              ...
              <button
                key={i}
                onClick={() => { setActiveModal(null); showSuccess(`${reward.type} sent to Chinedu Stores successfully!`); }}
                className="w-full flex items-center gap-3 p-4 bg-slate-50 hover:bg-amber-50 border border-transparent hover:border-amber-200 rounded-2xl transition-all cursor-pointer text-left"
              >
                <span className="text-xl">{reward.icon}</span>
                <div>
                  <p className="text-sm font-bold text-slate-900">{reward.type}</p>
                  <p className="text-[10px] text-slate-400">{reward.desc}</p>
                </div>
                <ArrowRight className="w-4 h-4 text-slate-400 ml-auto" />
              </button>
            ))}
          ...
          <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100">
            <p className="text-xs text-blue-700 leading-relaxed">💡 Rewarding loyal customers increases repeat purchase rate by an average of 34% based on SquadMind data.</p>
          </div>
        </div>
      </Modal>
      ...
      <aside className="w-[260px] bg-[#001f3f] flex flex-col justify-between shrink-0 h-full overflow-y-auto hidden md:flex">
        <div>
          ...
          <nav className="px-4 space-y-1">
            ...
            <button onClick={() => onNavigate('alerts')} className="w-full flex items-center gap-3 px-4 py-3 bg-[#112f4d] text-white rounded-lg transition-colors cursor-pointer">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              {visibleAlerts.map((alert) => (
                <div
                  ...
                  style={{ borderLeftWidth: '4px', borderLeftColor: alert.color }}
                >
                  <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <div
                  key={i}
                  onClick={() => {
                    if (kpi.label === 'ANOMALIES') onNavigate('frauddetection');
                    else if (kpi.label === 'HEALTH SCORE') onNavigate('trustscore');
                  }}
                  className={`bg-white rounded-[32px] p-10 shadow-sm border border-slate-100 flex flex-col items-start relative overflow-hidden group ${kpi.label !== 'TOTAL SCANNED' ? 'cursor-pointer hover:shadow-md hover:border-[#00d2ff]/30 transition-all' : ''}`}
                >
                  ...
                  {kpi.label === 'HEALTH SCORE' ? (
                    <div className="flex items-center gap-6 w-full">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-[#00d2ff] font-bold mt-2">Click to view TrustScore →</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      {kpi.label === 'ANOMALIES' && (
                        <p className="text-[10px] text-[#00d2ff] font-bold mb-4">Click to view Fraud Detection →</p>
                      )}
